...
$('#save_charges_form').submit(function(e) {
    e.preventDefault();
    var formData = new FormData($("#save_charges_form")[0]);
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            if (response.status == 'success') {
                $('form#save_charges_form').trigger('reset');
...
$(document).ready(function() {
    table = $('#charges_table').DataTable({
        "ajax": frontend_path + "receptionist/display_all_charges_data",
        "columns": [{
                "data": null
            },
            {
...
function change_status(status, id) {
    ...
    $.ajax({
        url: frontend_path + "receptionist/change_diseases_status",
        type: "POST",
        data: {
            'id': user_id,
            'status': user_status
